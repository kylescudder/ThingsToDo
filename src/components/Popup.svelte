...
<div class="flex flex-shrink-0 items-center justify-between p-4">
	<h5 class="text-xl font-medium leading-normal text-gray-800 dark:text-white" id="exampleModalLabel">{title}</h5>
	<button type="button"
  class="btn-close box-content w-4 h-4 p-1 hover:opacity-75" aria-label="Close">
  <i class="fas fa-xmark text-lg text-black dark:text-white dark:hover:text-white hover:text-black"
...
<div class="flex flex-shrink-0 flex-wrap items-center justify-end p-4 rounded-b-md">
	...
	<button type="button" on:click={() => modal.set(null)} class="btnCloseModal" data-bs-dismiss="modal">Close</button>
</div>